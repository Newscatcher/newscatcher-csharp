...
namespace NewscatcherApi.Test.Unit.MockServer.LatestHeadlines;
...
public class GetTest : BaseMockServerTest
{
    ...
    public async Task MockServerTest() {
        ...
        Server.Given(WireMock.RequestBuilders.Request.Create().WithPath("/api/latest_headlines").WithParam("when", "7d").WithParam("lang", "en,es").WithParam("not_lang", "fr,de").WithParam("countries", "US,CA").WithParam("not_countries", "UK,FR").WithParam("predefined_sources", "top 50 US, top 20 GB").WithParam("sources", "nytimes.com,finance.yahoo.com").WithParam("not_sources", "cnn.com,wsj.com").WithParam("not_author_name", "John Doe, Jane Doe").WithParam("parent_url", "wsj.com/politics,wsj.com/tech").WithParam("all_links", "https://aiindex.stanford.edu/report,https://www.stateof.ai").WithParam("all_domain_links", "who.int,nih.gov").WithParam("all_links_text", "Nvidia,Tesla").WithParam("word_count_min", "300").WithParam("word_count_max", "1000").WithParam("page", "2").WithParam("page_size", "50").WithParam("clustering_threshold", "0.7").WithParam("theme", "Finance,Tech").WithParam("not_theme", "Crime,Sports").WithParam("ORG_entity_name", ""Apple Inc" OR Microsoft").WithParam("PER_entity_name", ""Elon Musk" OR "Jeff Bezos"").WithParam("LOC_entity_name", ""San Francisco" OR "New York City"").WithParam("MISC_entity_name", "AWS OR "Microsoft Azure"").WithParam("title_sentiment_min", "-0.5").WithParam("title_sentiment_max", "0.5").WithParam("content_sentiment_min", "-0.5").WithParam("content_sentiment_max", "0.5").WithParam("iptc_tags", "20000199,20000209").WithParam("not_iptc_tags", "20000205,20000209").WithParam("iab_tags", "Business,Events").WithParam("not_iab_tags", "Agriculture,Metals").WithParam("custom_tags", "Tag1,Tag2").UsingGet())

        .RespondWith(WireMock.ResponseBuilders.Response.Create()
        .WithStatusCode(200)
        .WithBody(mockResponse));

        var response = await Client.LatestHeadlines.GetAsync(new GetLatestHeadlinesRequest {
            When = "7d",
            ByParseDate = true,
            Lang = "en,es",
            NotLang = "fr,de",
